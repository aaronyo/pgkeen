...
function makePool(hostConfig, poolConfig) {
  ...
  function connection(fn) {
    return config.getPromise().resolve(
      pgPool.connect().then(conn => {
        return config.getPromise().try(fn, conn).finally(conn.release);
      }),
    );
  }

  function close() {
    return config.getPromise().resolve(pgPool.end());
  }
...
class Connection {
  ...
  queryRaw(...args) {
    this.emitter.emit('received', args);
    if (usesNamedParams(args)) {
      args = [parameterize(...args)];
    }
    this.emitter.emit('formatted', args);
    const Promise = config.getPromise();
