...
function query(pgClient, sql, values, opts) {
  assert(
    !fp.has('text', opts),
  ...
  return pgClient.query({ text: sql, values, ...opts });
}
...
function namedParamsFunc(queryFn, sql, conversion, opts) {
  return async (pgClient, params) =>
    conversion(
      await queryFn(pgClient, ...namedParamsToBindVars(sql, params), opts),
    );
}
